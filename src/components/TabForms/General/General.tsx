...
import { GeneralData } from "./General.types";
import { Field, FieldArray, Form, Formik, FormikHelpers } from "formik";
import { generalValidationSchema } from "../../../utils/validationSchema";
import { Alert, Button, IconButton, OutlinedInput } from "@mui/material";
...
const GeneralForm = ({ onSubmit }: { onSubmit: (values: GeneralData) => void }) => {
    const [initialValues, setInitialValues] = useState<GeneralData>(initialValuesData);
    const [isFormVisible, setIsFormVisible] = useState<boolean>(false);

    const { onFileUpload, error } = useFileUpload<GeneralData>((json) => {
        setInitialValues(json);
        setIsFormVisible(true);
    });

    const onGeneralSubmit = (values: GeneralData, { resetForm }: FormikHelpers<GeneralData>) => {
        onSubmit(values);
        resetForm();
    };

    const renderGeneralForm = () => (
        <Formik initialValues={initialValues} validationSchema={generalValidationSchema} onSubmit={onGeneralSubmit}>
            {({ values, errors }) => (
                <>
                        ...
                        <div className="general-form__wrapper">
                                            ...
                                            {values.header.links.map((link, linkIndex) => (
                                                <div key={linkIndex} className="general-fields__section">
                                                    <div className="general-field-list__header">
                                                        <h4 className="general-field__title">Link {linkIndex + 1}</h4>
                            ...
                            <div>
                                ...
                                <div className="general-fields__section">
                                    ...
                                    />
                                    <CustomField label="Date" id="main-rangeBlock-date" name={`main.rangeBlock.date`} />
                                </div>
                                    ...
                                    {({ push: pushCard, remove: removeCard }) => (
                                        <>
                                            {values.forWhat.descriptionBlock.card_list.map((card, cardIndex) => (
                                                <div key={card.id} className="general-fields__section">
                                                    <div className="general-field-list__header">
                                                        <h4 className="general-field__title">Card {cardIndex + 1}</h4>
                                                        <div className="close-btn">
                                                            <IconButton
                                        ...
                                        <>
                                            {values.forWhat.priceBlock.buttons.map((btn, btnIndex) => (
                                                <div key={btnIndex} className="general-fields__section">
                                                    <div className="general-field-list__header">
                                                        <h4 className="general-field__title">Button {btnIndex + 1}</h4>
                                                        <div className="close-btn">
                                                            <IconButton
    ...
    return (
        <>
            ...
            {isFormVisible ? renderGeneralForm() : <FileUploadButton fileName="general" onFileUpload={onFileUpload} />}
        </>
    );
};
...
const initialValuesData: GeneralData = {
    common: {
        primaryButtonText: "",
        rangeData: {
            summa: {
